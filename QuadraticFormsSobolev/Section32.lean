...
integral) and its almost-everywhere convergence to `Φ`, which is Lemma A.2. -/
...
(3)), then the hypothesis, then Theorem 1.1; the last step is the circularity.
...
This is the lower bound of (2) used in the one direction that costs nothing:
...
sum times `h^{2d}`.** This is the identity that turns the sums of `(15)`
...
when it writes `(15)` as an inequality between integrals of `g_h` and
`g̃_h`. -/
...
/-- **`(15)`, in integral form.** Corollary 3.1 transported along
...
/-- `f_h(x_h(s)) → f(s)` almost everywhere: Lemma A.2, in the notation of
Section 3.2. -/
...
/-- **The left-hand integrand of `(15)` converges**, at almost every pair
...
/-- The discretized integrand of `(15)` is measurable. -/
...
everywhere: Lemma A.2 on the product, transferred to `ℝ≥0∞`. -/
...
/-- The integrand of the right-hand side of `(15)`, written as a function
...
/-- **The limit of the right-hand side of `(15)`.** For a function whose
...
/-- **Corollary 3.6 as assumption (4)**, with the separation parameter `3√d`
...
theorem formHs_ball_le_form_of_formHs_ne_top {ϑ α Λ : ℝ} (hϑ : 0 < ϑ)
  ...
  have hle1 : ∀ n, hn n ≤ R := fun n => by
    ...
    nlinarith [hR.le]
  -- the two sides of `(15)`, as functions of `n`
  set L : ℕ → ℝ≥0∞ := fun n =>
    ∫⁻ p : EuclideanSpace ℝ (Fin d) × EuclideanSpace ℝ (Fin d),
...
left of `(15)` then needs only that `h^{-α}·tileOsc` stay bounded along a
sequence. -/
...
the left of `(15)` and the splitting bound on the right, with no a priori
